...
import { Volume2, VolumeX, Home, Settings, LogIn, LogOut } from 'lucide-react';
...
export function LanguageToggle() {
  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      >
        <span className="inline-block min-w-[9.5rem] text-left">
...
export function UserWidget() {
  ...
  // Show a neutral placeholder while Firebase resolves the initial auth state
  // so the header doesn't flash between "Sign In" and the avatar.
  if (loading) {
    return (
      <div
        aria-hidden="true"
        className="h-9 w-28 animate-pulse rounded-full bg-slate-300/60"
      />
    );
  }
  ...
  if (!user) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 rounded-full border-2 border-slate-900/70 bg-white/90 px-3 py-1.5 font-serif font-black text-slate-950 shadow-lg backdrop-blur transition-colors hover:bg-white"
        data-testid="button-sign-in"
      >
        <LogIn size={15} aria-hidden="true" />
        <span className="text-sm">{t('sign_in')}</span>
      </button>
    );
  }
  ...
  return (
    <div ref={menuRef} className="relative">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={showMenu}
        className="flex items-center gap-2 rounded-full border-2 border-slate-900/70 bg-white/90 pl-1 pr-3 py-1 shadow-lg backdrop-blur transition-colors hover:bg-white"
        data-testid="button-user-menu"
      >
        {/* Avatar circle */}
        <span
          aria-hidden="true"
          className="flex h-7 w-7 items-center justify-center rounded-full bg-amber-500 text-xs font-black text-white"
        >
          {initial}
        </span>
        {/* Truncated display name */}
        <span className="max-w-[7rem] truncate font-serif font-black text-slate-950 text-sm leading-none">
          {displayName}
        </span>
        <motion.span
          aria-hidden="true"
        ...
        {showMenu && (
          ...
          >
            <button
              ...
              className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition-colors hover:bg-red-50 hover:text-red-700"
...
export function Header() {
  ...
  return (
    <motion.header
      ...
      className="pointer-events-none fixed left-0 right-0 top-0 z-50 flex items-center justify-between px-2 pt-2 sm:p-4"
    >
