...
export default function Home() {
  ...
  return (
    <main className="game-home min-h-[100dvh] overflow-x-hidden px-4 pb-10 pt-24 sm:px-6 sm:pt-20">
      ...
      >
        <motion.div
          ...
          className="game-logo mb-3 w-[82%] rounded-[1.5rem] border-[3px] border-amber-200/90 px-3 py-2.5 text-center"
        >
          <div className="mb-1 flex items-center justify-center gap-2 text-amber-100">
            <Sparkles size={17} />
            <span className="font-serif text-[10px] font-bold uppercase tracking-[0.25em] sm:text-xs">
              {t('company_name')}
            </span>
            <Sparkles size={17} />
          </div>

          <h1 className="game-title font-serif text-3xl font-black leading-none sm:text-4xl">
            {t('app_name')}
          </h1>

          <div className="mt-1.5 flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-amber-50/95">
            <BookOpen size={18} />
            <span>{t('tagline')}</span>
          </div>
        </motion.div>
